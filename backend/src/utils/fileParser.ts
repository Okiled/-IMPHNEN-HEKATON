...
export function parseFlexibleDate(dateInput: string | number | Date): string | undefined {
 ...
 */
export function parseCSV(content: string): ParsedSalesRow[] {
  ...
  if (colMap.product === -1 || colMap.quantity === -1) {
    ...
    const hasHeader = headers.some(h => 
      COLUMN_PATTERNS.product.some(p => h.toLowerCase().includes(p)) ||
 ...
 */
export function parseExcel(buffer: Buffer): ParsedSalesRow[] {
  const result: ParsedSalesRow[] = [];
 ...
 */
export function parseDOCX(buffer: Buffer): ParsedSalesRow[] {
  const result: ParsedSalesRow[] = [];
 ...
 */
export function parseFile(buffer: Buffer, filename: string): ParsedSalesRow[] {
